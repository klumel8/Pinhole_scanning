...
gridcir=false(gridrow,gridcol,floor(floor(numberCircles/part(2))/(1+large))); %devide by 100 for large res
...
for k = a:floor(floor(final-a)/(1+large))+a %devide by 100
    ...
    for gamma = 1:numberSteps
        ...
        gridcir(ceil(thetal/rowleng),ceil(phil/colleng),k-start)=1;
