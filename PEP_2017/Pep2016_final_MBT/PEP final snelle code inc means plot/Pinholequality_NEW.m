...
%variabelen voor PET-posities
xsmin=-50;
xsmax=46;
nxstap=2;   %nxstap= aantal stappen x
sgx=8;       %(xsmax-xsmin)/nxstap; %sgx= stapgrootte x
sgz=4;       %sgz = stapgrootte z
...
Lx =100; Ly = 54; Lz = 100;             % Size of volume
dx =2; dy =2; dz =2;                   % Step size for voxels

gridrow = 129; %19;                           % Gridsize: number of sections in the vertical direction
gridcol = 256; %32;                           % Gridsize: number of sections in the horizontal direction
...
meanplot(ii,:) = [mean(quality(:)) mean(temp(:)) median(quality(:)) min(temp(:)) median(temp(:)) max(quality(:))];
